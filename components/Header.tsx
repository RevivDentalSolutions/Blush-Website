...
export function Header(){const [open,setOpen]=useState(false);return <header className="site-header"><div className="nav-shell"><Link className="wordmark" href="/" onClick={()=>setOpen(false)}><span>{site.shortName}</span><small>Ink & Beauty Studio</small></Link><button className="menu-button" onClick={()=>setOpen(!open)} aria-expanded={open} aria-controls="primary-nav"><span className="sr-only">Toggle menu</span><i/><i/></button><nav id="primary-nav" className={open?"nav-open":""} aria-label="Primary navigation"><ul>{links.map(([l,h])=><li key={h}><Link href={h} onClick={()=>setOpen(false)}>{l}</Link></li>)}</ul><BookingLink className="btn btn-dark nav-book" source="header">Book now</BookingLink></nav></div></header>}
